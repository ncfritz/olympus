...
import { NZBFailureError } from "../../error/nzbFailureError";
import parse from "../../nzb/parser";
import { type StartDownloadMessage } from "../../types/messages";
import {
  DOWNLOAD_PREFIX,
...
@Injectable()
export class StartDownloadHandler extends BaseDownloadHandler {
  ...
  public async handle(msg: StartDownloadMessage, amqMsg: ConsumeMessage) {
    ...
    try {
      ...
      logger.debug(`NZBGet response: ${JSON.stringify(rpcResponse.data)}`);
      ...
      try {
        ...
        logger.error(
          `Error updating download: ${innerError.message}`,
          innerError,
        );
      }
    }
