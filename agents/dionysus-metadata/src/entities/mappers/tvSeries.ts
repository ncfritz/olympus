...
import { TV_SERIES_ID_TYPES } from "./externalIds";
import { UniqueSet } from "./UniqueSet";
import type { TmdbClient } from "../../tmdb/services/TmdbClient";

/** Maps TMDB's responses for a TV series to the API entity. */
export const toTvSeries = (
  seriesResponse: Awaited<ReturnType<TmdbClient["getTvSeriesDetails"]>>,
  recommendationsResponse: Awaited<
    ReturnType<TmdbClient["getTvSeriesRecommendation"]>
  >,
): PartialTvSeries => {
  ...
  const originCountries: UniqueSet<PartialCountryAssociation> = new UniqueSet();
