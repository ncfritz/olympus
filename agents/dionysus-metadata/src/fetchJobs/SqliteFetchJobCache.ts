...
import type { CacheConfigType } from "../config/configuration";
import { cacheConfig } from "../config/configuration";
import type { FetchJobCache } from "./FetchJobCache";

const logger = new Logger("SqliteFetchJobCache");

/**
 * The fetch job cache in SQLite (`<DIONYSUS_CACHE_PATH>/jobCache.db`): one
 * connection per process, opened on first use. Cache errors are logged and
 * treated as misses.
 */
@Injectable()
export class SqliteFetchJobCache implements FetchJobCache, OnModuleDestroy {
   ...
   * @throws when the cache directory can't be created
   */
  async open(): Promise<void> {
    ...
    try {
      ...
      logger.error(
        "Could not initialize SQLite database for cache.",
  ...
  async put(key: string, entry: MetadataFetchJob): Promise<void> {
    ...
    try {
      ...
      logger.error(
        `Error persisting entity "${key}" to cache`,
        e instanceof Error ? e.stack : String(e),
      );
    }
  }
