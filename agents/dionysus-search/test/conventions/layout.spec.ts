...
describe("agent conventions", () => {
  ...
  it("keeps handlers in handlers/ and services in services/", () => {
    const misplaced = SOURCES.filter((file) => {
      const name = path.basename(file, ".ts");
      const folder = path.basename(path.dirname(file));
      return (
        (/Handler$/.test(name) && folder !== "handlers") ||
        (/(Client|Service)$/.test(name) && folder !== "services")
      );
    });
  ...
  it("subscribes with the messaging constants, not string literals", () => {
    const literal = SOURCES.filter((file) =>
      [...read(file).matchAll(/@RabbitSubscribe\(([\s\S]*?)\)\n/g)].some(
        ([, body]) => /["'`]/.test(body),
      ),
    );
