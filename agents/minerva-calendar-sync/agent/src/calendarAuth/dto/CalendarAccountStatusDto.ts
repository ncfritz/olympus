...
const STATUS_VALUES: CalendarAccountAuthStatus[] = [
  "ok",
  "expired",
  "reauth_pending",
  "not_connected",
...
export class CalendarAccountStatusDto {
  @ApiProperty({
    description:
      "The stored-credential label this status describes (see google-credential-store / microsoft-credential-store)",
  })
  accountLabel: string;
  ...
  @ApiProperty({
    description:
      "Status of the device-flow OAuth credential backing this account's sync",
    enum: STATUS_VALUES,
  })
  ...
  @ApiProperty({
    description:
      "OAuth scopes granted the last time this account completed the login flow",
    required: false,
  })
  scope?: string;
