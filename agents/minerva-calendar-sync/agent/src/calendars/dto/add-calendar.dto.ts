...
export class AddCalendarDto {
  ...
  @ApiProperty({
    description:
      "Which stored OAuth credential this calendar authorizes with — must already be a connected account for this provider",
  })
  @IsString()
  @IsNotEmpty()
  accountLabel: string;
  ...
  @ApiProperty({
    description:
      "The provider's own calendar id, from GET /calendar-accounts/{accountLabel}/available-calendars?provider=...",
  })
  @IsString()
  @IsNotEmpty()
  calendarId: string;
