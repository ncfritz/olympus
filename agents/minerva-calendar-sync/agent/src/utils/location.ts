 ...
 *
 *   setLocation(response, request, DescribeCalendarAccountAuthorizationController, {
 *     authorizationId,
 *   })
 *   // -> /v1/calendar-account-authorization/<id>
 *
 * Ported from the Olympus API (apps/api/src/utils/location.ts); the agent's
 * routes have no domain segment. A reverse proxy that mounts the API under a prefix says so
...
export const locationOf = (
  ...
  return `${forwardedPrefix(request)}/v${version}${path.startsWith("/") ? "" : "/"}${path}`;
};
