...
describe('Auth (e2e)', () => {
  ...
  it('GET /auth/me returns the authenticated user', async () => {
    ...
    const res = await request(app.getHttpServer()).get('/auth/me').set('Authorization', authHeader).expect(200);
  ...
  it('rejects a valid token for an email no longer on the allowlist', async () => {
    ...
    await request(app.getHttpServer()).get('/auth/me').set('Authorization', authHeader).expect(403);
  });
  ...
  it('POST /auth/refresh exchanges a refresh token for a new access token', async () => {
    const { refreshToken } = app.get(AuthTokenService).issueTokenPair(E2E_ALLOWED_EMAIL);

    const res = await request(app.getHttpServer()).post('/auth/refresh').send({ refreshToken }).expect(200);
  ...
  it('POST /auth/refresh rejects an access token used as a refresh token', async () => {
    ...
    await request(app.getHttpServer()).post('/auth/refresh').send({ refreshToken: accessToken }).expect(401);
  });
