...
describe('Calendar management (e2e)', () => {
  ...
  it('POST /calendars 404s for an account that is not connected', async () => {
    await request(app.getHttpServer())
      ...
      .send({ provider: 'google', accountLabel: 'never-connected', calendarId: 'cal-x', source: 'X' })
      .expect(404);
  });
  ...
  it('POST /calendars adds a calendar for a known, connected account, and GET /calendars includes it', async () => {
    ...
    const list = await request(app.getHttpServer()).get('/calendars').set('Authorization', authHeader).expect(200);
    expect(list.body).toEqual(
      expect.arrayContaining([expect.objectContaining({ calendarId: 'cal-added-1', source: 'Added One' })]),
    );
  });
  ...
  it('DELETE /calendars/:calendarId removes a previously-added calendar', async () => {
    ...
    expect(list.body).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ calendarId: 'cal-added-2' })]),
    );
  });
