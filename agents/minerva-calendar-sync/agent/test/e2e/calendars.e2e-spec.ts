...
describe('Calendars (e2e)', () => {
  ...
  it('GET /calendars returns the configured calendars with sync status', async () => {
    ...
    const res = await request(app.getHttpServer()).get('/calendars').set('Authorization', authHeader).expect(200);
    expect(res.body).toEqual([{ ...FAKE_CALENDAR, synced: false, enabled: true, includedInBusy: true, syncing: expect.any(Boolean) }]);
  });
  ...
  it('PUT /calendars/:calendarId/enabled persists the toggle and GET /calendars reflects it', async () => {
    ...
    const res = await request(app.getHttpServer()).get('/calendars').set('Authorization', authHeader).expect(200);
    expect(res.body).toEqual([{ ...FAKE_CALENDAR, synced: false, enabled: false, includedInBusy: true, syncing: expect.any(Boolean) }]);
    ...
    const reenabled = await request(app.getHttpServer())
      ...
      .set('Authorization', authHeader)
      .expect(200);
    expect(reenabled.body).toEqual([{ ...FAKE_CALENDAR, synced: false, enabled: true, includedInBusy: true, syncing: expect.any(Boolean) }]);
  });
  ...
  it('PUT /calendars/:calendarId/included-in-busy persists the toggle and GET /calendars reflects it', async () => {
    ...
    const res = await request(app.getHttpServer()).get('/calendars').set('Authorization', authHeader).expect(200);
    expect(res.body).toEqual([
      { ...FAKE_CALENDAR, synced: false, enabled: true, includedInBusy: false, syncing: expect.any(Boolean) },
    ]);
    ...
    expect(reincluded.body).toEqual([
      { ...FAKE_CALENDAR, synced: false, enabled: true, includedInBusy: true, syncing: expect.any(Boolean) },
    ]);
  });
});
