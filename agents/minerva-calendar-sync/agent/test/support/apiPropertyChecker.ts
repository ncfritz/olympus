 ...
 *
 * Ported from packages/model/test/support/apiPropertyChecker.ts; the only
 * changes are `sourceDir`, which limits the check to the agent's src/model,
 * and resolving option spreads (`...CALENDAR_PROVIDER_ENUM`) to the const
 * object they name.
 *
...
export function checkApiProperties(
  packageRoot: string,
  sourceDir = "src",
): Finding[] {
  ...
  const srcRoot = path.join(packageRoot, sourceDir);
  ...
  for (const sf of program.getSourceFiles()) {
    ...
    function checkProperty(cls: string, prop: ts.PropertyDeclaration) {
      ...
      const decorator = readDecorator(prop, checker);
...
function readDecorator(
  ...
  for (const decorator of ts.getDecorators(prop) ?? []) {
    ...
    if (arg && ts.isObjectLiteralExpression(arg)) {
      collectOptions(arg, checker, options);
    }
    return {
...
/**
 * Property assignments of an options object, with `...NAME` spreads of a
 * const object literal resolved in place (later keys win, as at runtime).
 */
function collectOptions(
  ...
  for (const p of literal.properties) {
    if (ts.isPropertyAssignment(p)) {
      ...
      let symbol = checker.getSymbolAtLocation(p.expression);
      ...
      if (
        ...
        collectOptions(declaration.initializer, checker, options);
      }
    }
  }
}
