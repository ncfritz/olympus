...
describe("AuthTokenService", () => {
  ...
  it("round-trips a token pair", () => {
    const { accessToken, refreshToken } = tokens.issueTokenPair("me@example.com");
    expect(tokens.verifyAccessToken(accessToken)).toBe("me@example.com");
  ...
  it("rejects a refresh token presented as an access token", () => {
    ...
    expect(() => tokens.verifyAccessToken(refreshToken)).toThrow(/access token/);
  });

  it("rejects an access token presented as a refresh token", () => {
    const accessToken = tokens.issueAccessToken("me@example.com");
    expect(() => tokens.verifyRefreshToken(accessToken)).toThrow(/refresh token/);
  });

  it("rejects a garbage token", () => {
    expect(() => tokens.verifyAccessToken("not-a-jwt")).toThrow(/Invalid or expired/);
  });

  it("rejects a token signed with a different secret", () => {
    const other = new AuthTokenService(new JwtService({ secret: "different-secret" }));
    const accessToken = other.issueAccessToken("me@example.com");
    expect(() => tokens.verifyAccessToken(accessToken)).toThrow(/Invalid or expired/);
  });
  ...
  it("rejects an expired token", () => {
    ...
    const expiredToken = raw.sign({ email: "me@example.com", type: "access" }, { expiresIn: "-1s" });
    expect(() => tokens.verifyAccessToken(expiredToken)).toThrow(/Invalid or expired/);
  });
});
