...
import { mapGoogleEventToCanonical, resolveGoogleRemoval } from "../../../../src/providers/google/google-event-mapper";
...
describe("mapGoogleEventToCanonical", () => {
  ...
  it("defaults a missing summary to '(No title)'", () => {
    const result = mapGoogleEventToCanonical(baseEvent({ summary: undefined }), ctx, null);
  ...
  it.each([
    ...
    const result = mapGoogleEventToCanonical(baseEvent({ visibility }), ctx, null);
    expect(result.sensitivity).toBe(expected);
  });

  it("defaults importance to normal", () => {
    expect(mapGoogleEventToCanonical(baseEvent(), ctx, null).importance).toBe("normal");
  });
  ...
  it("classifies occurrenceType from recurringEventId / recurrence", () => {
    expect(mapGoogleEventToCanonical(baseEvent(), ctx, null).occurrenceType).toBe("single");
    expect(
      mapGoogleEventToCanonical(baseEvent({ recurringEventId: "master-1" }), ctx, null).occurrenceType,
    ).toBe("occurrence");
    expect(
      mapGoogleEventToCanonical(baseEvent({ recurrence: ["RRULE:FREQ=WEEKLY"] }), ctx, null).occurrenceType,
    ).toBe("series_master");
  });

  it("carries Google's recurringEventId through as recurrenceId", () => {
    const result = mapGoogleEventToCanonical(baseEvent({ recurringEventId: "master-1" }), ctx, null);
    expect(result.recurrenceId).toBe("master-1");
  });

  it("classifies type from attendee count, overridden by a non-default eventType", () => {
    expect(mapGoogleEventToCanonical(baseEvent(), ctx, null).type).toBe("appointment");
    expect(
    ...
    expect(mapGoogleEventToCanonical(baseEvent({ eventType: "outOfOffice" }), ctx, null).type).toBe("other");
  });

  it("derives reminder from useDefault or overrides", () => {
    expect(mapGoogleEventToCanonical(baseEvent(), ctx, null).reminder).toBe(false);
    expect(
      mapGoogleEventToCanonical(baseEvent({ reminders: { useDefault: true } }), ctx, null).reminder,
    ).toBe(true);
    expect(
      mapGoogleEventToCanonical(
  ...
  it("maps response: organizer takes priority over attendee status", () => {
    const result = mapGoogleEventToCanonical(baseEvent({ organizer: { self: true } }), ctx, null);
  ...
  it("defaults response to accepted with no organizer flag and no self attendee", () => {
    expect(mapGoogleEventToCanonical(baseEvent(), ctx, null).response).toBe("accepted");
  });

  it("maps free/busy status from transparency", () => {
    expect(mapGoogleEventToCanonical(baseEvent(), ctx, null).status).toBe("busy");
    expect(mapGoogleEventToCanonical(baseEvent({ transparency: "transparent" }), ctx, null).status).toBe(
      "free",
    );
  });

  it("maps cancelled from Google's event status field", () => {
    expect(mapGoogleEventToCanonical(baseEvent({ status: "cancelled" }), ctx, null).cancelled).toBe(true);
  });

  it("derives a uid from Google's id when iCalUID is missing (legacy/malformed events)", () => {
    const result = mapGoogleEventToCanonical(baseEvent({ iCalUID: undefined, id: "raw-id-1" }), ctx, null);
  ...
  it("derives uid from the instance's own id, not the series' shared iCalUID, for a recurring occurrence", () => {
    ...
    const first = mapGoogleEventToCanonical(
      baseEvent({ id: "series-1_20260105", iCalUID: "series-1@google.com", recurringEventId: "series-1" }),
      ctx,
      null,
    );
    const second = mapGoogleEventToCanonical(
      baseEvent({ id: "series-1_20260112", iCalUID: "series-1@google.com", recurringEventId: "series-1" }),
      ctx,
      null,
    );
  ...
  it("throws when both id and iCalUID are missing", () => {
    expect(() =>
      mapGoogleEventToCanonical(baseEvent({ id: undefined, iCalUID: undefined }), ctx, null),
    ).toThrow(/no id/);
  });

  it("throws when start or end is missing", () => {
    expect(() => mapGoogleEventToCanonical(baseEvent({ start: undefined }), ctx, null)).toThrow();
  });
  ...
  it("carries the resolved recurrenceRule straight through onto the canonical event", () => {
    ...
    expect(mapGoogleEventToCanonical(baseEvent(), ctx, "RRULE:FREQ=WEEKLY;BYDAY=MO").recurrenceRule).toBe(
      "RRULE:FREQ=WEEKLY;BYDAY=MO",
    );
    expect(mapGoogleEventToCanonical(baseEvent(), ctx, null).recurrenceRule).toBeNull();
  });
});

describe("resolveGoogleRemoval", () => {
  it("derives uid from iCalUID for a standalone event's removal", () => {
    const result = resolveGoogleRemoval({ id: "raw-id-1", iCalUID: "ical-uid-1@google.com", status: "cancelled" });
    expect(result).toEqual({ uid: "ical-uid-1@google.com", isOccurrence: false });
  });
  ...
  it("derives uid from the instance's own id, ignoring iCalUID, for an occurrence's removal", () => {
    ...
    expect(result).toEqual({ uid: "series-1_20260105@google.com", isOccurrence: true });
  });

  it("falls back to the id-derived convention when iCalUID is absent", () => {
    const result = resolveGoogleRemoval({ id: "raw-id-1", status: "cancelled" });
    expect(result).toEqual({ uid: "raw-id-1@google.com", isOccurrence: false });
  });

  it("throws when id is missing", () => {
    expect(() => resolveGoogleRemoval({ status: "cancelled" })).toThrow(/no id/);
  });
});
