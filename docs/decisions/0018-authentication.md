...
- Later: an ACME-capable subordinate CA (step-ca) under the internal root
  for short-lived certificates, replacing manual issuance.
...
- Access tokens: JWTs signed with ES256, 10 minutes, with `sub` (user),
  `client_id`, `aud` (`olympus-api`), `roles` and `kid`. The public keys
...
7. **Later.** step-ca under the internal root for short-lived
   certificates; the iOS app on the client package.
